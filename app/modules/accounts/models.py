...
class UserAccount(Base):
    ...
    __table_args__ = (
        UniqueConstraint("supabase_user_id", name="uq_user_accounts_supabase_user_id"),
    )

    id: Mapped[uuid.UUID] = mapped_column(
        Uuid(as_uuid=True), primary_key=True, default=uuid.uuid4
    )
    supabase_user_id: Mapped[str] = mapped_column(String(128), nullable=False)
    ...
    provider_subject: Mapped[str] = mapped_column(String(160), nullable=False, default="")
...
class LearnerProfile(Base):
    ...
    user: Mapped[UserAccount] = relationship(back_populates="learner_profile")
