...
class PretestEvidenceEvaluator:
    def evaluate(
        self,
        session: Session,
        *,
...
def _evidence_score(
    *,
    ...
    has_reasoning = reasoning_score is not None
    has_canvas = canvas_score is not None
    if has_reasoning and has_canvas:
        return round((0.60 * answer_score) + (0.25 * reasoning_score) + (0.15 * canvas_score), 4)
    if has_reasoning:
        return round((0.70 * answer_score) + (0.30 * reasoning_score), 4)
    if has_canvas:
        return round((0.70 * answer_score) + (0.30 * canvas_score), 4)
    return answer_score
